...
std::atomic<int> enc_n{0};
std::atomic<int> exe_n{0};
timeval buf_start;
timeval buf_end;
std::atomic<int> true_n{0};
std::atomic<int> update_time1{0};
std::atomic<int> update_time2{0};
//add end
Node* root=new Node();
//HZC
CTSL::HashMap<int,vector<myPkg>> rst_list;
moodycamel::ConcurrentQueue<myPkg> qry_rst;
myPkg query_pkg;
...
void searchRangeByCipher(int Sd,string c1,string c2,myPkg& pkg)
{
    if(root != nullptr && root->right!=nullptr)
    {
        cout<<"query by cipher:"<<c1<<","<<c2<<endl;
        Node* tmp=root->right;
        Node* parent=nullptr;
        int msg_id=0;
        int64_t cmp=0;
        ...
        while(1)
        {
            ...
            {
                myPkg rec_rst1;
                bool found=qry_rst.try_dequeue(rec_rst1);
                if(found && rec_rst1.msgid()==msg_id)
                {
                    msg_id++;
                    cmp=rec_rst1.rst();
                    break;
...
void traverseByCodeUDZ(int Sd,Node *root)
{
    ...
    traverseByCodeUDZ(Sd,root->left);
    traverseByCodeUDZ(Sd,root->right);
}
...
void searchRangeByCipherUDZ(int Sd, string c1, string c2,myPkg& pkg)
{
    if(root != nullptr && root->right!=nullptr)
    {
        ...
        int64_t k1=0,k2=0;
        searchOneByCipherUDZ(Sd,c1,k1,left_range);
        searchOneByCipherUDZ(Sd,c2,k2,right_range);
        Node* parent=lowestCommonAncestor(root->right,left_range,right_range);
        traverseByCodeUDZ(Sd,parent);
...
void searchRangeByCodeUDZ(int Sd,int64_t k1,int64_t k2,myPkg& pkg)
{
    if(root != nullptr && root->right!=nullptr)
    {
        Node* left_range=nullptr;
        Node* right_range=nullptr;
        searchOneByCodeUDZ(Sd,k1,left_range);
        searchOneByCodeUDZ(Sd,k2,right_range);
        Node* parent=lowestCommonAncestor(root->right,left_range,right_range);
//        cout<<parent->cipher;
        traverseByCodeUDZ(Sd,parent);
        traverseByCode(parent,k1,k2,pkg);
