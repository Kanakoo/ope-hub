...
//            send_msg(Sd,pkg);
            ...
            clients[qry_pkg.encid()%clients.size()]->send(str);
            while(1)
            {
                vector<myPkg> tmpv;
...
void searchOneByCipherUDZ(int Sd,string c,int64_t& k,Node*& p,int& msg_id,int qry_id)
{
    if(root != nullptr && root->right!=nullptr)
    {
        ...
        while(1)
        {
            ...
            std::string str;
            pkg.SerializeToString(&str);
            clients[qry_id%clients.size()]->send(str);
            while(1)
            {
                vector<myPkg> tmpv;
	    ...
	    k=parent->code;
            if(cmp>0)
            {
                ...
                if(l_size>0)
                {
                    ...
                    std::string str;
                    udz_item.SerializeToString(&str);
                    clients[qry_id%clients.size()]->send(str);
                    while (1) {
                        vector<myPkg> tmpv;
...
void searchOneByCodeUDZ(int Sd,int64_t k,Node*& p,int& msg_id,int qry_id)
{
    if(root != nullptr && root->right!=nullptr)
    {
        ...
        if(parent!=nullptr && cmp!=0)
        {
			...
			cout<<"send"<<qry_id<<","<<qry_id%clients.size()<<endl;
                    std::string str;
                    udz_item.SerializeToString(&str);
                    clients[qry_id%clients.size()]->send(str);
                    while (1) {
...
//        cout<<"----------encode by m0:"<<enc.cipher<<endl;
        ...
        else
        {
            ...
            if(parent!=nullptr)
            {
		timeval lockst,lockend;
		gettimeofday(&lockst,NULL);
                while (parent->lock==1) {
		    usleep(1);
                }
...
//                        compute_one(1,cipherx,parent,udz_item);
                        string str_udz;
                        udz_item.SerializeToString(&str_udz);
                        clients[enc_id%clients.size()]->send(str_udz);
	                return 0;
                    }
                    //handle end
                    y2=parent->code;
...
//                tx.mtd=1;
                ...
                tx_item.set_mtd(1);
                string str_tx;
                tx_item.SerializeToString(&str_tx);
                clients[enc_id%clients.size()]->send(str_tx);
//                send_msg(Sd,tx_item);
//                tx_buffer.enqueue(tx);
                return 0;
